...
an F test that is significant, meaning that the overall average mean of all vegetation zones is differently than zero. We also have a global F test on
`zone`, which is the p-value on the lower right, which suggests that we can
reject the null hypothesis that the means of different vegetation zones are
all the same. There is also a suggestion of which zone is significantly
different than others (`P`). Note that this summary of the fitted ANOVA model
uses Type I sums of squares.

Another standard `R` method for evaluating an ANOVA model is to use the
`anova()` function with our model to get an ANOVA summary for this model. This
also uses Type 1 sums of squares.
...
Now lets use the `car` way, which gives Type III sums of squares (Darren
recommends this).

```{r}
car::Anova(lm.ANOVA, type = "III", test.statistic = "F")
```
We see the same significant test that the intercept of the model (overall means
of the zones are different than zero), and the same global F test for `zone`
(some zones are significantly different than one another). In the case of this
moodel, the results are same with the Type III sums of squares as with Type I.
...
including information about the normality of the residuals. These can be
accessed by plotting the fitted model itself using `plot()`. For ANOVA, we are
primarily interested in the Q-Q normal plot, which we will examing for the
residuals of our linear model.
...
In this plot the divergence from a 1:1 line suggests that the residuals of the model are not completely normal, a violation of one of our ANOVA assumptions. In this
figure there are only a couple of outliers from the 1:1 line, so the residuals
may be more-or-less normally distributed. Lets also test this directly using the
Shapiro-Wilk test.
...
They do this using something called *random effects*.
