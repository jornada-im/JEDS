...
First, in this lesson we're learning to make inferences using statistics in this
lesson. *Inferential* statistics is a class of analysis where the objective is
to use samples drawn from a larger population to make inferences, or draw
logical conclusions, about the larger population. To do this, we commonly go
through a process of *hypothesis testing* where the objective is to identify
"significant" differences between samples by ruling out chance or sampling error
as the most plausible explanation for those differences. Hypothesis tests are
usually framed with two competing hypotheses:

1. The null hypothesis ($H_0$) that there is no statistical relationship.
2. The alternative hypothesis ($H_1$) that there is one or more statistical
...
**Global F-test for a fixed-effect in ANOVA**
  ...
  * $H_1$: at least one level has a different mean than another level
...
Lets get our ANOVA summary results two ways. First, the standard `R` way is to
use the `anova()` function with our model to get an ANOVA summary for this 
model. This uses Type 1 sums of squares.

```{r}
anova(lm.ANOVA)
```
We have a low p-value for the global F test for `zone` here, which suggests that
we can reject the null hypothesis that the means of different vegetation zones
are the same.
...
```
The intercept here is an F test on the overall average difference between the
means of all vegetation zones and zero. There is also an F test for `zone` where
the results are same with the Type III sums of squares as with the Type I.
