...
linear regression, one- and two-way analysis of variance (ANOVA), and analysis of covariance (ANCOVA). All of these are variations of "straight-line-fitting"
...
effect on the mean NPP values. The information in the ANOVA summary table isn't
enough to determine which vegetation zones have means that are *different than
each other*.

Making comparisons between groups is typically something we do after the ANOVA
analysis, and is called a *post-hoc* comparison or test^[As with other
...
the p-values indicating that we can reject the null hypothesis that the means
...
We also ignored the somewhat unequal variance among groups in our observation
though we know that it exists. As we look at additional years of data, we'll
have to reconsider both of these assumptions when fitting linear models.
...
1. there is unequal variance between groupings in the data
