...
2. Press the little green "New" button to create a new repository

3. Name the repository `my_gh_repo`

4. Initialize with a `README.md` file

Now your repository is created. To move the repository to your local machine,
you will do something called "cloning" the GitHub repository. First lets make a
place on your local machine to put the repository. Go back to your system shell
and make a new directory called `GitHub` to hold local clones of your GitHub
repositories.
...
Now you need to use `git` to clone the repository on GitHub using its web
address. Returning to your repository page (<https://github.com/{username}/my_repo>),
look in the top right of the new repository's landing page, and there is a green
"Code" button. Click it to open a dialogue that gives you the option to copy an
address. Use the "SSH" option and press the "Copy" button to the right of the
...
```{bash, clone_GitHub, eval=FALSE}
cd ~/GitHub/
git clone git@github.com:gremau/my_gh_repo.git
```

Now you should have a clone, or copy, of the repository you just made on GitHub
ready to work with in your local `GitHub` directory.
...
echo "This repository does this" >> ~/GitHub/my_gh_repo/README.md
echo "This repository does that" >> ~/GitHub/my_gh_repo/README.md
```
...
"origin", or the repository on GitHub that we cloned from. If we are content
with these changes we can stage them and commit them.
...
Great! But now our local clone of the repository has a commit that is not at 
GitHub. How do we fix this? First lets look at what `git status` says.

```{bash, new_repo_status2}
git status
```

Our local repository is one commit ahead of the origin now. To bring the GitHub
repository up to date, we need to `git push` the changes there. Note that this
isn't going to work unless you have configured SSH access to GitHub. For now,
this is beyond the scope of this tutorial, so [see here](https://swcarpentry.github.io/git-novice/07-github/index.html#3-ssh-background-and-setup) to learn how to set up SSH access.
